...
class FrontendSnippetStorage
{
    ...
    public function directory(string $appName, string $version): ?string
    {
        ...
        $this->logger->info('Frontend snippets of app "{appName}" loaded from its source', ['appName' => $appName, 'location' => $source->location]);
        ...
        if ($stored === null) {
            try {
                ...
                $this->logger->error('Could not persist frontend snippets of app "{appName}"', ['appName' => $appName, 'exception' => $e]);
            }
        }
