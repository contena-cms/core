<?php declare(strict_types=1);

namespace Contena\Core\System\Channel\File\Discovery;

use Contena\Core\Framework\Adapter\Twig\TemplatePathIteratorInterface;
use Contena\Core\Framework\Util\Hasher;
use Contena\Core\System\Channel\File\ChannelFileCacheInvalidator;
use Symfony\Component\Mime\MimeTypes;
use Symfony\Contracts\Cache\CacheInterface;
use Symfony\Contracts\Cache\ItemInterface;

/**
 * @internal
 */
class ChannelFileDiscovery
{
    public function __construct(
        private readonly TemplatePathIteratorInterface $templateIterator,
        private readonly CacheInterface $cache,
    ) {
    }

    /**
     * @return array<string, ChannelFile>
     */
    public function discover(string $fileFamily = ChannelFile::DEFAULT_FILE_FAMILY): array
    {
        return $this->cache->get(
            'channel-file-discovery-' . Hasher::hash($fileFamily),
            function (ItemInterface $item) use ($fileFamily): array {
                $item->expiresAfter(null);
                $item->tag(ChannelFileCacheInvalidator::buildDiscoveryCacheTag());

                return $this->discoverUncached($fileFamily);
            }
        );
    }

    public function get(string $templatePath): ?ChannelFile
    {
        $fileFamily = $this->extractFileFamily($templatePath);
        if ($fileFamily === null) {
            return null;
        }

        $fileName = $this->extractFileName($fileFamily, $templatePath);
        if ($fileName === null) {
            return null;
        }

        return $this->discover($fileFamily)[mb_strtolower($fileName)] ?? null;
    }

    /**
     * @return array<string, ChannelFile>
     */
    private function discoverUncached(string $fileFamily): array
    {
        $files = [];
        foreach ($this->catalogueRegisteredFiles($fileFamily) as $normalizedFileName => $templatePaths) {
            $templatePath = $templatePaths[0];
            $fileName = $this->extractFileName($fileFamily, $templatePath);
            \assert($fileName !== null);

            $files[$normalizedFileName] = new ChannelFile(
                $fileFamily,
                $fileName,
                $templatePath,
                $this->resolveContentType($fileName),
                $templatePath,
                [],
                $templatePaths,
            );
        }

        return $files;
    }

    /**
     * @return array<string, list<string>>
     */
    private function catalogueRegisteredFiles(string $fileFamily): array
    {
        $paths = [];
        $templatePathPrefix = ChannelFile::TEMPLATE_ROOT . '/' . $fileFamily . '/';

        // Template paths come from registered Twig templates; request path validation happens before loading public files.
        foreach ($this->templateIterator->getTemplatePathsForSubPath($templatePathPrefix, true) as $templatePath) {
            if (!str_ends_with($templatePath, ChannelFile::TEMPLATE_SUFFIX)) {
                continue;
            }

            $fileName = mb_substr($templatePath, mb_strlen($templatePathPrefix), -mb_strlen(ChannelFile::TEMPLATE_SUFFIX));
            $paths[mb_strtolower($fileName)][$templatePath] = $templatePath;
        }

        ksort($paths);

        foreach ($paths as $normalizedFileName => $templatePaths) {
            ksort($templatePaths, \SORT_STRING);
            $paths[$normalizedFileName] = array_values($templatePaths);
        }

        return $paths;
    }

    private function extractFileFamily(string $templatePath): ?string
    {
        $prefix = ChannelFile::TEMPLATE_ROOT . '/';

        if (!str_starts_with($templatePath, $prefix)) {
            return null;
        }

        $fileFamily = mb_substr($templatePath, mb_strlen($prefix));
        $position = mb_strpos($fileFamily, '/');

        if ($position === false) {
            return null;
        }

        return mb_substr($fileFamily, 0, $position);
    }

    private function extractFileName(string $fileFamily, string $templatePath): ?string
    {
        $prefix = ChannelFile::TEMPLATE_ROOT . '/' . $fileFamily . '/';

        if (!str_starts_with($templatePath, $prefix) || !str_ends_with($templatePath, ChannelFile::TEMPLATE_SUFFIX)) {
            return null;
        }

        return mb_substr($templatePath, mb_strlen($prefix), -mb_strlen(ChannelFile::TEMPLATE_SUFFIX));
    }

    private function resolveContentType(string $fileName): string
    {
        $extension = strtolower(pathinfo($fileName, \PATHINFO_EXTENSION));
        $contentType = MimeTypes::getDefault()->getMimeTypes($extension)[0] ?? 'text/plain';

        return $contentType . '; charset=utf-8';
    }
}
