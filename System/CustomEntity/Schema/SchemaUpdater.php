 ...
 * @internal
 *
 * @phpstan-type CustomEntityField array{name: string, type: string, required?: bool, translatable?: bool, reference: string, inherited?: bool, onDelete: string, channelApiAware?: bool, ignoreMissingReference?: bool, default?: mixed}
 */
class SchemaUpdater
{
        ...
        foreach ($fields as $field) {
            ...
            switch ($field['type']) {
                ...
                case 'many-to-many':
                    ...
                    // build mapping table name: `custom_entity_blog_blogs`
                    ...
                    // add reference id column: `blog_id`
                    ...
                    if (!$reference->hasColumn('version_id')) {
                        ...
                        // add foreign key to source table (custom_entity_blog.id <=> custom_entity_blog_blogs.custom_entity_blog_id), add cascade delete for both
                        ...
                        // add foreign key to reference table (blog.id <=> custom_entity_blog_blogs.blog_id), add cascade delete for both
                ...
                case 'one-to-one':
                    ...
                    // add inheritance column which matches the association name: `blog.customEntityBlogTopSeller`
                    $this->addInheritanceColumn($schema, $name, $field);
                ...
                case 'one-to-many':
                    ...
                    // important: we add a `comment` to the column. This allows us to identify the custom entity modification in ct-core tables when run the cleanup
                    $reference->addColumn($foreignKey, Types::BINARY, $fieldOptions + $binary + ['comment' => self::COMMENT]);

                    // build foreign key with special naming. This allows us to identify the custom entity modification in ct-core tables when run the cleanup
