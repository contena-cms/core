...
use Contena\Core\Framework\DataAbstractionLayer\Field\Flag\Extension as DalExtension;
...
use Contena\Core\Framework\DataAbstractionLayer\Field\PriceField;
...
class DynamicFieldFactory
{
    ...
    public static function create(ContainerInterface $container, string $entityName, array $fields): FieldCollection
    {
        ...
        foreach ($translated as &$field) {
            ...
            $apiAware = $field['channelApiAware'] ?? false;
    ...
    private static function defineField(array $field, FieldCollection $collection, string $entityName, ContainerInterface $container): void
    {
        ...
        $apiAware = ($field['channelApiAware'] ?? false) ? new ApiAware() : null;
        ...
        switch ($field['type']) {
            ...
            case 'many-to-many':
                ...
                // build mapping name:   'custom_entity_blog_blogs'  => use field name instead of reference entity name to allow multiple references to same entity
                ...
                // field is maybe flag to be channel-api aware
                self::addFlag($association, $apiAware);

                // check blog inheritance and add ReverseInherited(reverse-property-name)
                ...
                // reverse property schema: #table#_#column# -  custom_entity_blog_blogs
                ...
                // check for blog inheritance use case
                if ($reference->isInheritanceAware() && $inherited) {
            ...
            case 'many-to-one':
                ...
                // add flag for channel-api awareness
                self::addFlag($association, $apiAware);

                // check for blog inheritance use case and define reverse inherited flag. Used when joining from custom entity table to blog table
                if ($reference->isInheritanceAware() && $inherited) {
                ...
                // in sql we define the on-delete flag on the foreign key, for the DAL we need the flag on the reverse side, so we can check which association are affected when deleting the record (e.g. blog)
