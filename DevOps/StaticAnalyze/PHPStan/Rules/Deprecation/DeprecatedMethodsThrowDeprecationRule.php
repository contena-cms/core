...
namespace Contena\Core\DevOps\StaticAnalyze\PHPStan\Rules\Deprecation;
...
class DeprecatedMethodsThrowDeprecationRule implements Rule
{
     ...
     */
    private const array RULE_EXCEPTIONS = [
        ...
        if ($classDeprecation && !$this->isServiceConstructor($node, $class) && !$this->handlesDeprecationCorrectly($classDeprecation, $methodContent)) {
            return [
                RuleErrorBuilder::message(\sprintf(
                    ...
                    ->identifier('contena.deprecatedClass')
                    ->build(),
            ];
        }
        ...
        if (!$deprecationOfParentMethod && $methodDeprecation && !$this->handlesDeprecationCorrectly($methodDeprecation, $methodContent)) {
            return [
                RuleErrorBuilder::message(\sprintf(
                    ...
                    ->identifier('contena.deprecatedMethod')
                    ->build(),
            ];
        }
