...
{
    public const string ERROR_MESSAGE = 'assertEquals() on Closure instances no longer does structural comparison since PHPUnit 12 — it falls back to identity (object hash), so two separately-constructed closures with identical behavior are unequal. Assert on the result of calling the closure instead.';
            ...
            if ($closureType->isSuperTypeOf($argType)->yes()) {
                return [
                    RuleErrorBuilder::message(self::ERROR_MESSAGE)
                        ->identifier('contena.assertEqualsOnClosure')
                        ->build(),
                ];
            }
        }
