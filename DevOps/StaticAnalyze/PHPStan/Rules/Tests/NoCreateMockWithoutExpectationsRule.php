...
{
    public const string ERROR_STUB = 'createMock(%s) is only used as a stub (no ->expects() is configured on it). Use createStub(%s) instead, the correct PHPUnit API for a test double without call expectations.';
    ...
    private function buildError(Node $createMockCall, int $line, string $message): RuleError
    {
        ...
        return RuleErrorBuilder::message(\sprintf($message, $label, $label))
            ->identifier('contena.createMockWithoutExpectations')
            ->line($line)
            ->build();
    }
