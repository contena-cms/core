...
{
    public const string ERROR_MESSAGE = 'assertEqualsCanonicalizing() argument #%d must be a list so the comparison value-sorts: use a list literal or array_values()/array_keys()/array_column(). For keyed arrays or objects use assertEquals() instead — canonicalizing only affects lists.';
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        foreach ([0, 1] as $position) {
            ...
            $errors[] = RuleErrorBuilder::message(\sprintf(self::ERROR_MESSAGE, $position + 1))
                ->identifier('contena.assertEqualsCanonicalizingListArgument')
                ->build();
        }
