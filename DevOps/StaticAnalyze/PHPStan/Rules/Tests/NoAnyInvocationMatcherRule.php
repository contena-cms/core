...
{
    public const string ERROR_REDUNDANT = 'The any() invocation matcher is redundant: expects($this->any()) places no constraint on the call count, so it is equivalent to omitting expects() entirely. Drop it and call ->method()/->willReturn() directly on the mock.';
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        return [
            RuleErrorBuilder::message(self::ERROR_REDUNDANT)
                ->identifier('contena.phpunitAnyMatcher')
                ->build(),
        ];
    }
}
