...
class NoDropStatementInUpdateRule implements Rule
{
    ...
    private const string MIGRATION_METHOD_NAME_TO_BE_CHECKED = 'update';
    private const string DROP_TABLE_REGEX_PATTERN = '/DROP\s+TABLE\s+`?\w+`?/i';
    private const string DROP_COLUMN_REGEX_PATTERN = '/ALTER\s+TABLE\s+`?\w+`?\s+DROP\s+COLUMN\s+`?\w+`?/i';
    private const string DROP_FOREIGN_KEY_REGEX_PATTERN = '/ALTER\s+TABLE\s+`?\w+`?\s+DROP\s+FOREIGN\s+KEY\s+`?\w+`?/i';
    private const string DROP_REGEX_PATTERN = '/ALTER\s+TABLE\s+`?\w+`?\s+DROP\s+(?!INDEX|CONSTRAINT|DEFAULT)`?\w+`?/i';
    private const int SW_MAJOR_VERSION_FROM_WHICH_ON_BLUE_GREEN_SHOULD_BE_CHECKED = 6;
    ...
    private function inspectMethodCall(MethodCall $statement, Identifier $name, ClassMethod $node, array $errors): array
    {
        if (\in_array($name->name, self::$disallowedMethodCalls, true)) {
            ...
            if ($name->name === 'dropForeignKeyIfExists') {
                $message .= ' Dropping FKs is OK if immediately re-added, or if not breaking old app version validation. Use @phpstan-ignore contena.dropStatement if intentional.';
            }

            $errors[] = RuleErrorBuilder::message($message)
                ->identifier('contena.dropStatement')
