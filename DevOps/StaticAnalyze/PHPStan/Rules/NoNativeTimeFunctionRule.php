...
class NoNativeTimeFunctionRule implements Rule
{
     ...
     */
    protected const string EXEMPT_PATH_PATTERN = '#(/Migration/|Hydrator\.php$|/Test/|/tests/|/Profiling/|Profiler|Telemetry|/DevOps/)#';

    private const string ERROR_MESSAGE = 'Do not use native time reads. They cannot be frozen in tests. Use Psr\Clock\ClockInterface to inject a controllable clock.';

    private const array NOT_ALLOWED_FUNCTIONS = [
        'time',
        'microtime',
        'hrtime',
        'strtotime',
    ...
    private function buildError(): array
    {
        return [
            RuleErrorBuilder::message(self::ERROR_MESSAGE)
                ->identifier('contena.noNativeTimeRead')
                ->build(),
        ];
    }
}
