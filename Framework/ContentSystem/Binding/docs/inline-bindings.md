...
```yaml
# MyPlugin/Resources/content-system/types/blog/quick-view.yaml
properties:
  relatedBlog:
    type: Contena\Core\Content\Blog\Channel\ChannelBlogEntity
    resolvedBy: relatedBlogId

bindings:
  with-media-association:
    label: "Related blog (with media)"
    resolves:
      relatedBlog:
        entity:
          property: relatedBlogId
          associations: [media]
```
