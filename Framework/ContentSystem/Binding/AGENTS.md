...
- `DatabaseBindingSpecificationLoader` deserializes all active-app rows and validates them together, keyed by `source:id` so equal bare ids from different apps remain distinct; a missing name or a schema that fails to decode, deserialize, or validate aborts the whole load, matching `YamlBindingSpecificationLoader`'s fail-fast behavior. A collective constraint violation from either loader is a `bindingSpecificationLoadValidationFailed` 500 with the `BINDING_SPECIFICATION_LOAD_FAILED` code, because definitions are server-side state; exceptions thrown by the validator itself propagate out of `load()` without being wrapped
