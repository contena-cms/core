...
- A worked layout combining entity rendering, data loading and context distribution — [docs/blog-detail-page.md](docs/blog-detail-page.md)
...
- **Entity Specification Sources**: `Content/Blog/Aggregate/BlogContentLayout/BlogSpecificationSource`, `Content/Category/Aggregate/CategoryContentLayout/CategorySpecificationSource`, `Content/LandingPage/Aggregate/LandingPageContentLayout/LandingPageSpecificationSource`
- **Header/Footer Sources**: `Frontend/ContentSystem/HeaderContentLayout/HeaderSpecificationSource`, `Frontend/ContentSystem/FooterContentLayout/FooterSpecificationSource`
...
- **Channel API**: `Channel/ContentRoute` (single class, DI-parameterized per format + section)
- **Admin Preview API**: `Api/ContentPreviewController` — one route minting a short-lived token for an unsaved draft layout and returning a Frontend preview URL that renders it against real entity data, assignment-free. Route, mint-time admission, the absent direct render route and the payload store's contract are in [Api/AGENTS.md](Api/AGENTS.md); the wire contract in [Api/docs/preview-url.md](Api/docs/preview-url.md)
...
- **Write-time gates**: the resolvability gate, the write-boundary default seeder and the delete protection — `Layout/LayoutDefaultSeeder` seeds each element's stored type defaults, including recursively collected defaults for nested members, and shares the per-type rule in `Layout/Type/StoredDefaultProvider` with mutations; see [docs/layout-write-gates.md](docs/layout-write-gates.md)
...
- OpenAPI schemas: update `src/Core/Framework/Api/ApiDefinition/Generator/Schema/ChannelApi/` when modifying endpoints
