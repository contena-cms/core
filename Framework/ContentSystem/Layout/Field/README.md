...
The DAL field type behind the `content_layout.layout` column, and its serializer. One field, one serializer, registered with the `contena.field_serializer` DAL tag so the framework's serializer registry discovers it.
