...
**Broadcast** - All children receive identical data (e.g., blog detail page with shared blog)

**Indexed** - Children receive data by position: child[N] gets data[N] (e.g., top 3 blogs in specific slots)
...
**Sliced** - Data split into chunks per child, with `sliceSize` items per chunk (default: `10`). E.g., 12 blogs with `sliceSize: 4` across 3 columns = 4 per column.

**Iterator** - Sequential distribution: each child receives one item in order. E.g., 10 blogs distributed to 10 card elements, one each.
...
```
Valid:
  Provider (blog detail)
    -- Consumer (title)          <-- Can receive

Invalid:
  Consumer (title)
    -- Provider (blog detail) <-- Cannot receive from child
