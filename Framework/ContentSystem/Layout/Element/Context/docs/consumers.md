...
Fields:
- Context key (`"blog"`) - Under `scope: parent`, matches the provider's context key (or its `consumerAlias`); under `scope: root`, matches a key of the layout's root-ambient context. Either way the match is the key itself or a dot path hanging below it (see [path-resolution.md](path-resolution.md))
