...
**The Problem:** You build reusable layout components (blog cards, content blocks, sliders) that need to work in different places - homepage grids, category listings, search results. When you nest these components inside container elements (grids, sections, columns), the container needs to pass data through to the nested components.

**Example scenario:** A blog grid contains blog cards. The grid receives blog data and needs to pass it to each card. Without redistribution, you must configure both `acceptsContext` (to receive data) AND `providesContext` (to pass it along) on the grid - verbose and repetitive.
...
"acceptsContext": {"blog": {"type": "single", "required": true}},
...
**Example:** Container receives `featuredBlog`, but child blog cards expect `blog`:
...
Container accepts `featuredBlog`, children receive `blog`. Reuse the same blog card components everywhere.
...
- Need specific nested properties like `blog.cover`
...
**Example:** Blog page > content section > blog card > title element
  ...
  "slots": {
    "main": [{
      ...
      "slots": {
        "content": [{
          "id": "blog-title",
          "acceptsContext": {"blog.name": {"type": "single", "required": true}}
        }]
      }
    }]
  }
...
The `content-section` container automatically passes blog data to nested components. Move this section to different pages - it still works.
