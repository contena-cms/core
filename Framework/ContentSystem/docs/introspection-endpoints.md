...
- **Style Option API**: `GET /api/_info/content-system-style-options.json` (`InfoController::getContentSystemStyleOptions()`), plus a folded `styleOptions` key on `content-system-element-types.json`. Wire schema: `style` on the ChannelApi `ContentElement` (omitted when empty)
