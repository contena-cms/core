...
A data-driven layout system for serving structured content through the Channel API. Layouts define element trees with data requirements and context distribution, rendered through an event-driven pipeline.
...
**Main** (`/channel-api/content/{path}`) — Entity-based rendering for Blogs, Categories, and Landing Pages. Layouts assigned per entity with channel fallback. See Adapter/.

**Header** (`/channel-api/content-header*`) — Domain-aware singleton per domain/channel. Three-tier fallback: domain+channel → channel → global. See Adapter/.

**Footer** (`/channel-api/content-footer*`) — Same domain-aware resolution as header.

Header and Footer are Frontend-owned sections: the Core ships none of their data wiring. The `ContentSection` enum that names them (`HEADER`, `FOOTER`, `MAIN`) lives in the Core, but their entity definitions, specification sources, and section resolvers are all registered by the Frontend module via `content-system.php`. This is intentional: headless deployments without the Frontend bundle operate without header/footer sections.

Each section supports four response formats: full, decomposed, skeleton, and data. See Channel/ and Output/.
...
**Domain-owned:** Entity definitions, specification sources, data loaders, config serializers. These are co-located with the domain entity they serve (e.g., blog data loader lives in the blog module).
...
- **Mutation/** - [Mutation/README.md](Mutation/README.md) - Server-side structural layout edits (insert, remove, move, replace, duplicate, wrap, unwrap, attach, bind, update properties), each re-resolved through the diagnostics pass; applied either statelessly to a draft tree or committed to a stored layout
...
- **Channel/** - [Channel/README.md](Channel/README.md) - Channel API endpoints
...
- **Frontend/ContentSystem/** - [Frontend/ContentSystem/README.md](../../../Frontend/ContentSystem/README.md) - Header and footer sections, which are Frontend-owned.
