...
namespace Contena\Core\Framework\Adapter;
...
class AdapterException extends HttpException
{
    ...
    public static function unknownRedisConnection(string $connectionName): self
    {
        return new self(
            ...
            'Can\'t provide connection "{{ connectionName }}", check if it\'s configured under contena.redis.connections.',
            [
    ...
    public static function invalidRedisConnectionDsn(string $connectionName): self
    {
        return new self(
            Response::HTTP_INTERNAL_SERVER_ERROR,
            self::REDIS_UNKNOWN_CONNECTION,
            'contena.redis.connections dsn of "%s" connection must be a string.',
            [
                'connectionName' => $connectionName,
            ],
        );
