...
use Contena\Core\Framework\Webhook\Event\WebhookActivationTrigger;
...
use Contena\Core\Framework\Webhook\Health\ErrorClassification;
use Contena\Core\Framework\Webhook\Health\HealthChange;
use Contena\Core\Framework\Webhook\Health\HealthConfig;
use Contena\Core\Framework\Webhook\Health\HealthRow;
use Contena\Core\Framework\Webhook\Health\SuspensionCause;
...
use Contena\Tests\Integration\Core\Framework\Webhook\Health\EndpointHealthStateMachineMatrixTest;
...
class WebhookHealthService
{
    ...
    /**
     * Keeps operator kills out of automated recovery.
     */
    ...
    private function disable(string $webhookId): int
    {
        $change = $this->transition($webhookId, function (HealthRow $row): ?HealthRow {
            if ($row->state === EndpointState::Disabled) {
                if ($row->disabledOrigin === DisabledOrigin::Operator) {
                    return null;
                }
    ...
    /**
     * The one write path: lock the row, let $decide compute the next row, store it and flip the
     * backlog. $decide returns null to leave the row untouched.
     *
