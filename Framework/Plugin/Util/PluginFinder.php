...
class PluginFinder
{
    final public const string COMPOSER_TYPE = 'contena-platform-plugin';
    private const string CONTENA_PLUGIN_CLASS_EXTRA_IDENTIFIER = 'contena-plugin-class';
    ...
    private function loadLocalPlugins(string $pluginDir, IOInterface $composerIO, ExceptionCollection $errors): array
    {
        ...
        try {
            ...
            foreach ($filesystemPlugins as $filesystemPlugin) {
                ...
                if (!$this->isContenaPluginType($package) || !$this->isPluginComposerValid($package)) {
    ...
    private function isContenaPluginType(CompletePackageInterface $package): bool
    {
    ...
    {
        return isset($package->getExtra()[self::CONTENA_PLUGIN_CLASS_EXTRA_IDENTIFIER])
            && $package->getExtra()[self::CONTENA_PLUGIN_CLASS_EXTRA_IDENTIFIER] !== ''
            && !empty($package->getExtra()['label']);
    }

    private function getPluginNameFromPackage(CompletePackageInterface $pluginPackage): string
    {
        return $pluginPackage->getExtra()[self::CONTENA_PLUGIN_CLASS_EXTRA_IDENTIFIER];
    }
    ...
    private function enrichWithVendorPlugins(
        ...
        foreach ($composerPackages as $composerPackage) {
            if (!$this->isContenaPluginType($composerPackage)) {
                continue;
            }
        ...
        if ($this->isContenaPluginType($root) && $this->isPluginComposerValid($root)) {
        ...
        $errors->add(new PluginComposerJsonInvalidException(
            ...
            [
                \sprintf(
                    ...
                    self::COMPOSER_TYPE,
                    self::CONTENA_PLUGIN_CLASS_EXTRA_IDENTIFIER
                ),
            ]
        ));
    }
