...
class AssetService
{
    private const string EXTENSION_RESOURCES_DIRECTORY = 'Resources/public';
    private const string ASSET_MANIFEST_FILENAME = 'asset-manifest.json';
    ...
    private function copyAssetsFromBundleOrApp(
        ...
        if (!EnvironmentHelper::getVariable('CONTENA_SKIP_ASSET_INSTALL_CACHE_INVALIDATION', false)) {
    ...
    {
        return $this->parameterBag->get('contena.filesystem.asset.type') === 'local';
    }

    private function getAssetVisibility(): string
    {
        try {
            return (string) ($this->parameterBag->get('contena.filesystem.asset.visibility') ?? Visibility::PUBLIC);
