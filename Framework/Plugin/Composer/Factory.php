...
namespace Contena\Core\Framework\Plugin\Composer;
...
class Factory
{
    public static function createComposer(string $composerJsonDir, ?IOInterface $composerIO = null): Composer
    {
        ...
        // This is a workaround to make sure that the contena platform package has the correct version
        if (($json['name'] ?? '') === 'contena/platform' && !isset($json['version']) && !EnvironmentHelper::hasVariable('COMPOSER_ROOT_VERSION')) {
            $_SERVER['COMPOSER_ROOT_VERSION'] = Kernel::CONTENA_FALLBACK_VERSION;
        }
