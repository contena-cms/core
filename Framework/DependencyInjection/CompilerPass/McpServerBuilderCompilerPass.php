 ...
 *    `addNotificationHandlers()` from one global tag for every server, but Contena's handlers are
 ...
 *    `addLoaders()` in place would newly advertise every app tool on /channel-api/_mcp too.
 * 3. Both servers page with Contena's own pagination parameter, so the number the allowlist request
...
class McpServerBuilderCompilerPass implements CompilerPassInterface
{
        ...
        foreach ($definition->getMethodCalls() as [$method, $arguments]) {
            switch ($method) {
                ...
                case 'setPaginationLimit':
                    $arguments = ['%contena.mcp.pagination_limit%'];
     ...
     * Contena builds the transport itself, so both endpoints would start answering it the moment the
