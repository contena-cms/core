    ...
    private function checkHealthEndpoint(Manifest $manifest, string $healthCheckUrl): ?UnmetRequirement
    {
        try {
            ...
            if ($response->getStatusCode() !== Response::HTTP_OK) {
                return $this->fail($manifest, \sprintf(
                    'Health check at "%s" returned HTTP %d. Ensure the Contena instance is running and publicly reachable.',
                    $healthCheckUrl,
            ...
            return $this->fail($manifest, \sprintf(
                'Could not reach "%s". Ensure the Contena instance is publicly accessible at the configured APP_URL.',
                $healthCheckUrl
            ));
