...
class DumpClassSchemaCommand extends Command
{
    ...
    private function getCollectionEntity(string $className): ?string
    {
        ...
        foreach ($findNodes as $findNode) {
            if ((string) $findNode->name === 'getExpectedClass') {
                ...
                \assert($classConst->class instanceof Name);

                $resolvedClass = (string) $classConst->class;
                \assert(class_exists($resolvedClass));
    ...
    /**
     * @param array<int|string, Stmt> $stmts
     *
    ...
    /**
     * @return array<int|string, Node>|null
     */
