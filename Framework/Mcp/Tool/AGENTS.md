...
## Naming
- Tool names use `contena-` prefix with kebab-case: `contena-entity-search`, `contena-entity-upsert`
...
- **Lead with the user's trigger phrases.** A description that opens with "The correct tool for count, sum, average, and other aggregate questions" routes correctly when the user asks "how many blogs?". A description that opens with "Run aggregations over any Contena entity" does not.
- **Use negative phrasing to break ties.** When two tools share keywords, the description must spell out the contrast: `"Use this — NOT contena-entity-search — for any 'how many'…"` is more decisive than a positive description alone.
- **Do not reference other tools as prerequisites unless they truly are.** Phrases like "Use contena-foo-read to check current values first" train the agent to call the read tool even when the user explicitly asked to write. Declare prerequisites with `#[McpToolDependsOn]`, not in prose.
- **Mention the use cases the user will name.** If a prompt is "upload this image to the media library", the description should contain the phrase "media library". Otherwise the agent often returns no tool selection at all.
- **Make required parameters truly required.** A parameter without a PHP default ends up `required: true` in the JSON schema. If users frequently won't supply it (a channel UUID, a tax ID), give it a default of `''` or `null` and validate inside `__invoke()`. GPT-4o refuses to call tools when required parameters are missing from the prompt — even when the description says they are optional.
...
#[McpTool(name: 'contena-entity-delete', description: '...')]
...
- `McpToolAnalysisCompilerPass` resolves dependencies transitively and stores the result in the `contena.mcp.tool_dependencies` container parameter, which the allowlist provider uses at runtime.
...
| Tool | Depends on |
|---|---|
| `contena-entity-read` | `contena-entity-schema` |
| `contena-entity-search` | `contena-entity-schema` |
| `contena-entity-aggregate` | `contena-entity-schema` |
| `contena-entity-upsert` | `contena-entity-schema` |
| `contena-entity-delete` | `contena-entity-search` (→ `contena-entity-schema`) |
| `contena-system-config-write` | `contena-system-config-read` |
...
#[McpTool(name: 'contena-system-config-read', description: '...')]
...
```php
#[McpTool(name: 'contena-entity-read', description: '...')]
...
|---|---|
| `contena-system-config-read` | `system_config:read` |
| `contena-system-config-write` | `system_config:update` |
| `contena-media-upload` | `media:create` |
| `contena-entity-read` | `<entity>:read` (dynamic) |
| `contena-entity-search` | `<entity>:read` (dynamic) |
| `contena-entity-aggregate` | `<entity>:read` (dynamic) |
| `contena-entity-upsert` | `<entity>:create`, `<entity>:update` (dynamic) |
| `contena-entity-delete` | `<entity>:delete` (dynamic) |
| `contena-theme-config` (Frontend) | `theme:read`, `theme:update` |
...
- `_meta` is optional, used for pagination (`total`, `page`, `limit`), context (`channelId`), and write metadata (`dryRun`)
...
| | `contena-entity-search` | `contena-entity-aggregate` |
|---|---|---|
...
| Typical use | "Show me the last 10 blogs" | "How many active members?" |

**Why not combine them?** Entity rows can already fill most of the inline 100 KB budget for typical searches. Bucket aggregations (like `terms` on blog titles or `date-histogram` by day) can produce hundreds or thousands of result entries on their own. Mixing both in one response means a single oversized result is much more likely to spill out of the inline budget into the resource cache, hiding the actual data behind a `resourceUri` round-trip. Separate tools remove the ambiguity: `entity-search` is always bounded by `limit`, `entity-aggregate` is always bounded by the aggregation definitions.
...
Entity tools (`EntitySearchTool`, `EntityReadTool`, and the plugin's `FrontendSearchTool`) auto-apply `includes` when the caller hasn't specified them. This dramatically reduces response size by only serializing the fields AI clients actually need.

**How it works:**
1. The tool introspects the `EntityDefinition` to find all scalar fields (id, name, position, etc.)
...
4. The `translated` pseudo-field is always injected for entities with `TranslatedField` instances, ensuring inherited/resolved values (e.g. translated blog titles) are never lost
...
All entity read tools use `JsonEntityEncoder` for serialization (not the Channel API serializer), so `includes`/`excludes` filtering works consistently regardless of whether the data comes from a regular or channel repository.

## Read tools
- `EntitySchemaTool` (`contena-entity-schema`) -- entity field/association introspection
- `EntitySearchTool` (`contena-entity-search`) -- criteria-based search; returns records only, **never aggregations** (see below)
- `EntityAggregateTool` (`contena-entity-aggregate`) -- aggregation-only queries (`limit: 0` internally, no entity rows in response)
- `EntityReadTool` (`contena-entity-read`) -- single entity read by ID
- `SystemConfigReadTool` (`contena-system-config-read`) -- read global or channel configuration

## Write tools
- `EntityUpsertTool` (`contena-entity-upsert`) -- create/update entities (dryRun wraps in transaction + rollback)
- `EntityDeleteTool` (`contena-entity-delete`) -- delete entities (dryRun shows cascade impact)
- `SystemConfigWriteTool` (`contena-system-config-write`) -- update configuration values
- `MediaUploadTool` (`contena-media-upload`) -- upload media from a URL to the media library
...
| Plugin (`contena.mcp.tool` DI tag) | `McpToolDiscoveryCompilerPass` re-tags it and assigns it to the Admin API server | Missing tag; wrong tag name; attribute on method instead of class |
