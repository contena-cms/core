 ...
 */
#[McpResource(
    uri: 'contena://extensions',
    name: 'contena-extensions',
...
class ExtensionsResource
{
     ...
     *
     * TODO: Replace with dynamic data from SBP (Contena Plugin Store) API
     *       so extensions are discovered automatically rather than hardcoded here.
     *
     * @return list<array{name: string, type: 'plugin'|'bundle', tool_prefix: string, description: string, install_command: string, documentation_url: string|null}>
     */
    ...
    private function resolveStatuses(array $extensions): array
    {
        ...
        foreach ($extensions as $extension) {
            $rows = $extension['type'] === 'plugin' ? $pluginRows : [];

            $statuses[$extension['name']] = match ($extension['type']) {
                'plugin' => isset($rows[$extension['name']])
