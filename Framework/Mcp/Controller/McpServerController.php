...
/**
 * Contena-aware entry point for the MCP protocol over HTTP.
 * Applies Contena's Admin API authentication and route scoping, then delegates
...
class McpServerController
{
    ...
    private const TOOL_SEARCH = 'contena-tool-search';
     ...
     * contena-toolset-enable). This runs after {@see Server::run()} has persisted the SDK's
    ...
    private function enrichInitializeResponse(Request $request, PsrResponseInterface $psrResponse): PsrResponseInterface
    {
        ...
        if (!$response->addContenaMeta($source->getUserId(), $source->getIntegrationId())) {
            return $psrResponse;
        }
