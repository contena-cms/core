...
Planning reference for contributors and PMs. For the public docs see [developer.contena.cn/docs](https://developer.contena.cn/docs/). For the in-repo contributor index see [README.md](README.md).
...
| **Merchant plugin** | All `merchant-*` tools in `CtMcpMerchantAssistant` (done); core keeps primitives only |
| **CtMcpDevTools MVP** | Log streaming + search shipped (done); declared installable |
| **Public docs** | First slice on developer.contena.cn shipped (done); in-repo kept as contributor reference |
| **Official samples** | `McpHelloWorld` + `CtMcpAdminUsers` moved to `contena/*` org, polished, linked from docs |

### V2 (post-SCD) — planned

| Theme | Outcome |
|-------|---------|
| **CtMcpDevTools depth** | System/health probes, deprecation/version context, migration status, log–request correlation |
...
| **Docs depth** | Exhaustive reference, automation, full site chapter on developer.contena.cn |
| **Analytics products** | In-admin dashboards, SIEM packs (requires V1 structured observability baseline) |
| **Channel API MCP** | Customer-authenticated MCP surface for buyer-journey tools (browsing, cart, checkout). Needs PM driver |
...
| Item | Status | Notes | Effort | Horizon |
|------|--------|-------|--------|---------|
| Bundle in `custom/bundles/CtMcpDevTools/` | **Done** | Reuses `/api/_mcp`, integration auth, ACL, rate limiter | M | — |
| Log streaming tool (`ct-dev-tools-log-stream`) | **Done** | Filter by channel/level/time; sensitive-field redaction | M | — |
| Log search tool (`ct-dev-tools-log-search`) | **Done** | Query by message pattern / correlation ID | S–M | — |
| Notifications tool (`ct-dev-tools-notifications`) | **Done** | Polls/SSE-waits for Contena notifications; `NotificationEventSubscriber` fires on indexer + import-export completion | S | — |
...
First public slice shipped via `contena/docs#2264`. In-repo `docs/` is now the contributor reference; canonical content is on developer.contena.cn.
...
| `CtMcpAdminUsers` plugin in-repo | **Done** | ACL-aware plugin with 2 tools + 2 resources | S | — |
...
Contena uses **Streamable HTTP** at `/api/_mcp` via `symfony/mcp-bundle`. Session init and JSON-RPC routing are delegated to the bundle/SDK. Contena adds: Admin API auth bridge, rate limits, feature flag, `McpContextProvider`, app HMAC execution, `McpToolCompilerPass`.
...
| **Resources** | 8 static resources + `contena://tool-result/{id}` template (large-result delivery) | Templates/subscriptions if clients rely on them; ACL policy still open |
...
- Apps: XML, DB, signed remote tools (`McpHelloWorld`, `CtMcpAdminUsers` in-repo).
- ACL + dry-run + per-integration allowlist + rate limit + conflict detection.
- **`CtMcpDevTools` MVP** — log streaming, log search, notifications tool (indexer/import-export events via SSE). Lives in `custom/bundles/CtMcpDevTools/`.
- **Merchant workflows out of core** — all 9 `merchant-*` tools in `custom/plugins/CtMcpMerchantAssistant/`; zero merchant tools remain in `src/Core/`.
...
- **Reference apps** — `custom/apps/McpHelloWorld/` and `custom/plugins/CtMcpAdminUsers/` exist in-repo (org move to `contena/*` still pending).
...
- **`contena/*` org move for samples** — `McpHelloWorld` and `CtMcpAdminUsers` are in-repo on this branch; need move + polish + canonical docs links.
...
| **B — MCP extensions** | WS2 (V2 depth), WS3, WS5, WS6 | `contena/*` org move for samples; CtMcpDevTools V2 probes |
