...
## Purpose
Loaders extend the MCP SDK's capability discovery to include tools, prompts, and resources from Contena apps. Three concrete loaders (`AppMcpToolLoader`, `AppMcpPromptLoader`, `AppMcpResourceLoader`) extend `AbstractAppMcpLoader`, which provides the shared try/fetch/foreach pipeline plus `capabilityName()` (prefixes the app name) and `resolveDescription()` helpers.

## Plugin integration
Plugins register MCP tools by tagging services with `contena.mcp.tool` in their DI XML. At compile time, `McpToolDiscoveryCompilerPass` does two things:

1. Re-tags the service `contena.mcp.tool` → `mcp.tool` so it is wired into the DI container and service locator.
...
<mcp xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    <mcp-tools>
        <mcp-tool name="publish-content" url="https://app.example.com/mcp/publish-content">
            <label>Publish content</label>
            <label lang="zh-CN">发布内容</label>
            <input-schema>
...
App tool names are automatically prefixed with the app name (e.g., `my-cms-publish-content`). If the resulting name starts with `contena-`, the tool is silently skipped and a warning is logged. This prevents apps from overriding built-in core tools.
...
```
- `installationId` identifies the Contena instance (from `InstallationIdProvider`)
...
- `AppMcpToolLoader` -- reads from `app_mcp_tool`, registers tools, enforces reserved `contena-` prefix, honors the `contena.mcp.allowed_tools` compile-time allowlist
