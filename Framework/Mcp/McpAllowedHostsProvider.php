 ...
 * variants. Contena is reached through its configured public hostnames, so those must be
 * allowlisted too — otherwise every non-local request is rejected with "Invalid Host header".
 * The hostnames are derived from APP_URL and every channel domain, keeping the mitigation
 * in place while allowing the platform's own hosts (Administration and Frontend/Channel API).
 */
class McpAllowedHostsProvider
{
    ...
    public function getAllowedHosts(): array
    {
        ...
        $urls = $this->connection->fetchFirstColumn('SELECT `url` FROM `channel_domain`');
