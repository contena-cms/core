...
class McpExceptionListener implements EventSubscriberInterface
{
    ...
    private const CHANNEL_API_MCP_ROUTE_NAME = 'channel-api.mcp.endpoint';
     ...
     * Symfony HTML 404 (or a redirect to a Member-register Frontend page) which they
    ...
    public function onException(ExceptionEvent $event): void
    {
        ...
        if (
            \in_array(
                ...
                [self::MCP_ROUTE_NAME, self::CHANNEL_API_MCP_ROUTE_NAME],
                true,
            )
        ) {
        ...
        if ($request->getPathInfo() === self::OAUTH_FALLBACK_PATH && $request->getMethod() === 'POST') {
            $event->setResponse(new JsonResponse([
                ...
                'error_description' => 'Authentication failed. Configure your MCP client with the correct Administration API integration credentials for /api/_mcp or Channel API credentials for /channel-api/_mcp.',
