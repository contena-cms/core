 ...
 * @codeCoverageIgnore
 *
 * @see \Contena\Tests\Integration\Core\Maintenance\System\Service\SetupDatabaseAdapterTest
 */
class SetupDatabaseAdapter
{
    ...
    public function initializeContenaDb(Connection $connection, ?string $database = null): bool
    {
        if (!$hasContenaTables = $this->hasContenaTables($connection, $database)) {
            $connection->executeStatement($this->getBaseSchema());
        }

        return !$hasContenaTables;
    }

    public function hasContenaTables(Connection $connection, ?string $database = null): bool
    {
        if ($database) {
