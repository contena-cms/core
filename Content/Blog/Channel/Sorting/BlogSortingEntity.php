...
class BlogSortingEntity extends Entity
{
    use EntityIdTrait;
    ...
    protected ?BlogSortingTranslationCollection $translations = null;
    ...
    public function createDalSorting(?FieldSorting $fallbackSorting = null): array
    {
        ...
        usort($fields, static fn (array $a, array $b): int => $b['priority'] <=> $a['priority']);
        ...
        if ($fallbackSorting !== null && !\in_array($fallbackSorting->getField(), $flat, true)) {
            $sorting[] = $fallbackSorting;
        }

        if (\in_array('id', $flat, true) || \in_array('blog.id', $flat, true)) {
            return $sorting;
        }
    ...
    public function getTranslations(): ?BlogSortingTranslationCollection
    {
        return $this->translations;
    }

    public function setTranslations(BlogSortingTranslationCollection $translations): void
    {
