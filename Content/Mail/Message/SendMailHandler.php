...
final readonly class SendMailHandler
{
    ...
    public function __invoke(SendMailMessage $message): void
    {
        ...
        try {
            ...
            if (!$this->filesystem->fileExists($mailDataPath)) {
                $this->logger->error('The mail data file does not exist. Mail could not be sent.', $this->logContext($message, ['exception' => $e->getMessage()]));
        ...
        if (!is_a($mail, Email::class)) {
            $this->logger->error('The mail data file does not contain a valid email object. Mail could not be sent.', $this->logContext($message));
    ...
    private function cleanup(SendMailMessage $message): void
    {
        ...
        try {
            ...
            $this->logger->error('Could not delete mail data file after sending mail.', $this->logContext($message, ['exception' => $e->getMessage()]));
        }
    }
     ...
     */
    private function logContext(SendMailMessage $message, array $context = []): array
    {
        return [
            'mailDataPath' => $message->mailDataPath,
