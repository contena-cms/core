...
CREATE TABLE IF NOT EXISTS `position` (
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `uniq.position.scope_code` (`data_scope_id`, `code`),
    KEY `idx.position.data_scope_id` (`data_scope_id`),
    KEY `idx.position.active_position` (`active`, `position`),
    CONSTRAINT `json.position.custom_fields` CHECK (JSON_VALID(`custom_fields`)),
    CONSTRAINT `fk.position.data_scope_id` FOREIGN KEY (`data_scope_id`)
...
CREATE TABLE IF NOT EXISTS `position_translation` (
    ...
    KEY `idx.position_translation.data_scope_id` (`data_scope_id`),
    ...
    CONSTRAINT `fk.position_translation.data_scope_id` FOREIGN KEY (`data_scope_id`)
...
CREATE TABLE IF NOT EXISTS `user_position` (
    ...
    KEY `idx.user_position.data_scope_id` (`data_scope_id`),
    ...
    CONSTRAINT `fk.user_position.position_id` FOREIGN KEY (`position_id`)
        REFERENCES `position` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT `fk.user_position.data_scope_id` FOREIGN KEY (`data_scope_id`)
        REFERENCES `data_scope` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
SQL);
    }
